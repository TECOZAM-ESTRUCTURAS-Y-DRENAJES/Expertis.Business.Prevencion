...
Public Class PrevencionObraDoc
    ...
    Private Const cnEntidad As String = "tbObraPrevDoc"
...
#Region "RegisterAddNewTasks"
    ...
    <Task()> Public Shared Sub AsignarValoresPredeterminados(ByVal data As DataRow, ByVal services As ServiceProvider)
        data("idDocumentacion") = AdminData.GetAutoNumeric
...
#Region " RegisterValidateTasks "
    ...
    <Task()> Public Shared Sub ValidarCamposObligatorios(ByVal data As DataRow, ByVal services As ServiceProvider)
        If Solmicro.Expertis.Engine.Length(data("idDocumentacion")) = 0 Then ApplicationService.GenerateError("El código de la documentacion de obra es un dato obligatorio.")
...
#Region "Funciones"
    Public Function UpdateForm(ByVal dr As DataRow, Optional ByVal iHistorico As Int16 = 0) As System.Data.DataTable
        ...
        Try
            ...
            Dim dtDoc = SelOnPrimaryKey(dr(cnEntidad & "_idDocumentacion"))
            If IsNothing(dtDoc) OrElse dtDoc.Rows.Count = 0 Then
                dtDoc = MyBase.AddNewForm
                dtDoc.Rows(0)("idDocumentacion") = AdminData.GetAutoNumeric()
            End If
            ' Control de historizar
            If iHistorico = 1 Then
                dtDoc.Rows(0)("snHistorico") = iHistorico
            End If
            ' Recorrer las columnas de la fila y asignar valores
            dtDoc.Rows(0)("idObra") = dr("idObra") ' <-- La obra de la cabecera
            For Each columna As DataColumn In dr.Table.Columns
                ...
                If scolumna.GetLength(0) > 1 Then
                    If scolumna.GetValue(0) = cnEntidad And columna.ColumnName <> cnEntidad & "_idDocumentacion" Then
                        dtDoc.Rows(0)(scolumna.GetValue(1)) = dr(columna.ColumnName)
                        ' Si es Historico
                        If iHistorico = 1 Then
    ...
    Public Function UpdateForm2(ByVal dr As DataRow, ByVal fecha As String, ByVal solicitante As String, ByVal solicita As String, ByVal envio As String, ByVal observaciones As String, Optional ByVal iHistorico As Int16 = 0) As System.Data.DataTable
        ' Recibe la fila del formulario y la trata
        Try
            Dim scolumna() As String
            ...
            If IsNothing(dtDireccion) Or dtDireccion.Rows.Count = 0 Then
                ...
                dtDireccion.Rows(0)("idDocumentacion") = AdminData.GetAutoNumeric()
            End If
            ' Control de historizar
            If iHistorico = 1 Then
            ...
            Try
                dtDireccion.Rows(0)("Fecha") = fecha
                dtDireccion.Rows(0)("Solicitante") = solicitante
                dtDireccion.Rows(0)("Solicita") = solicita
                dtDireccion.Rows(0)("ModEnvio") = envio
    ...
    <Task()> Public Shared Sub GenerarNuevaDoc(ByVal data As dataDoc, ByVal services As ServiceProvider)
        Dim NV As New PrevencionObraDoc
        Dim dtNV As DataTable = NV.AddNewForm
        Dim drNV As DataRow = dtNV.NewRow

        dtNV.Rows(0)("IDDocumentacion") = AdminData.GetAutoNumeric
        dtNV.Rows(0)("idObra") = data.idObra
        dtNV.Rows(0)("Fecha") = data.fecha
        dtNV.Rows(0)("ModEnvio") = data.envio
        dtNV.Rows(0)("Observaciones") = data.observaciones
        dtNV.Rows(0)("Solicita") = data.solcita
        dtNV.Rows(0)("Solicitante") = data.solicitante

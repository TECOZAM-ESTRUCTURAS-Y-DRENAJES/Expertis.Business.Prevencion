...
Public Class PrevencionEncargaJefe
    ...
    Private Const cnEntidad As String = "tbOperarioEncargaJefe"
...
#Region "RegisterAddNewTasks"
    ...
    <Task()> Public Shared Sub AsignarValoresPredeterminados(ByVal data As DataRow, ByVal services As ServiceProvider)
        data("IDEncargaJefe") = AdminData.GetAutoNumeric
...
#Region " RegisterValidateTasks "
    ...
    <Task()> Public Shared Sub ValidarCamposObligatorios(ByVal data As DataRow, ByVal services As ServiceProvider)
        If Solmicro.Expertis.Engine.Length(data("IDEncargaJefe")) = 0 Then ApplicationService.GenerateError("El código del encargado es un dato obligatorio.")
...
#End Region
    ...
    Friend Sub AdminEncargaJefe(ByVal snombre As String, ByVal sTipo As String)
        Try
            Dim DtAdmin As DataTable
            Dim DE As New DataEngine
            Dim FAdmin As New Filter
            FAdmin.Add("Nombre", snombre.ToUpper)
            DtAdmin = DE.Filter("tbOperarioEncargaJefe", FAdmin, "Nombre")
            'DtAdmin = AdminData.Filter("select nombre from tbOperarioEncargaJefe", , "Nombre = '" & snombre.ToUpper & "'", , False).DataTable

            If IsNothing(DtAdmin) OrElse DtAdmin.Rows.Count = 0 Then
                DtAdmin = MyBase.AddNewForm
                DtAdmin.Rows(0)("idEncargaJefe") = AdminData.GetAutoNumeric()
                DtAdmin.Rows(0)("Nombre") = snombre.ToUpper
                DtAdmin.Rows(0)("ctipo") = sTipo
                Me.Update(DtAdmin)
            End If
        Catch ex As Exception
            ApplicationService.GenerateError("Error al crear fila de encargados/jefes." & ex.Message)
        End Try
    End Sub
